...
then
	...
	trace-cmd report $trace|awk 'BEGIN{FS="."}{print $1$2}'|awk '{if(NF==10){print $3"\t"$1"\t"$2"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10"\n"}}' >> $temp
	if [ -n "$host_trace" ]
	then
		trace-cmd report $host_trace|awk 'BEGIN{FS="."}{print $1$2}'|awk '{if(NF==10){print $3"\t"$1"\t"$2"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10"\n"}}'|grep qemu-system-arm >> $temp
	fi
