...
do
	...
	echo_run	$nohup_run $user@$vm1_ip $vm_working_dir $notify_run $notify_info $log_to_dst $log_base/$single_vm_log_dir 2 $single_vm_frequency -h $host_ip -v $vm1_period:$vm1_exec -i $vm1_pid -s 1000000000 -t 1 -u 0 -p 1000 -l $log_base/$single_vm_log_dir 
	echo_run	$nohup_run $user@$vm2_ip $vm_working_dir $notify_run $notify_info $log_to_dst $log_base/$single_vm_log_dir 2 $single_vm_frequency -h $host_ip -v $vm2_period:$vm2_exec -i $vm2_pid -s 1000000000 -t 1 -u 0 -p 1000 -l $log_base/$single_vm_log_dir 
	echo_run	$notify_check -u $notify_user -a $notify_ip -p $notify_path -l 1 $vm1_ip $vm2_ip
		
	echo_run	$nohup_run $user@$vm1_ip $vm_working_dir $notify_run $notify_info $log_to_dst $log_base/$single_vm_log_dir 2 $single_vm_frequency -h $host_ip -v $vm1_period:$vm1_exec -i $vm1_pid -s 1000000000 -t 1 -u 50 -p 1000 -l $log_base/$single_vm_log_dir 
	echo_run	$nohup_run $user@$vm2_ip $vm_working_dir $notify_run $notify_info $log_to_dst $log_base/$single_vm_log_dir 2 $single_vm_frequency -h $host_ip -v $vm2_period:$vm2_exec -i $vm2_pid -s 1000000000 -t 1 -u 50 -p 1000 -l $log_base/$single_vm_log_dir 
	echo_run	$notify_check -u $notify_user -a $notify_ip -p $notify_path -l 1 $vm1_ip $vm2_ip
done
